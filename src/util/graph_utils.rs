...
use crate::{block::BlockId, block_store::BlockStore, edge_label::EdgeLabel};
...
#[allow(dead_code)]
pub fn extract_reachable_subgraph(
    graph: &Cfg,
    ...
    let entry = block_store
        .add_block_to_graph(&mut cfg, *graph.node_weight(entry_node).unwrap())
        .unwrap();
    ...
    while let Some(node) = worklist.pop_front() {
        for edge in graph.edges(node) {
            ...
            let succ_tree_node = *graph2cfg.entry(succ).or_insert_with(|| {
                block_store
                    .add_block_to_graph(&mut cfg, *graph.node_weight(succ).unwrap())
                    .unwrap()
            });
...
pub fn calc_post_dominator(cfg: &Cfg, virtual_exit_node: NodeIndex) -> Dominators<NodeIndex> {
    ...
    let post_dom = simple_fast(Reversed(&tmp_tree), virtual_exit_node);
...
{
    let mut worklist: Vec<NodeIndex> = nodes.into_iter().filter(|&n| Some(n) != except).collect();
